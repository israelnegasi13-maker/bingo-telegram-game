// game-logic.js - BINGO ELITE GAME LOGIC MODULE - OPTIMIZED VERSION
...
function cleanupProcessingClaims() {
  const now = Date.now();
  const fiveSecondsAgo = now - 5000; // Reduced from 10 seconds for faster cleanup
...
// ========== OPTIMIZED CHECKBINGO FUNCTION ==========
function checkBingo(markedNumbers, grid) {
  // Create a Set for faster lookups
  const markedSet = new Set(markedNumbers.map(m => m.toString()));
  
  // Check FREE space once
  const hasFree = markedSet.has('FREE');
  ...
  for (const patternData of patterns) {
    ...
    for (const index of patternData.pattern) {
      ...
      } else if (!markedSet.has(cellValue.toString())) {
        isBingo = false;
        break;
      }
    }
...
// ========== NEW: FAST BINGO WIN PROCESSING FUNCTION ==========
async function processBingoWin(winData) {
  const {
    claimId,
    userId,
    userName,
  ...
  console.log(`🚀 Processing bingo win for ${userName} in room ${room}`);
  
  try {
    // 1. Check if room is still valid (fast atomic update)
    ...
    if (!updatedRoom) {
      console.log(`⚠️ Room ${room} was already ended or user not in game`);
      return;
    }
    ...
    const basePrize = contributionPerPlayer * playersCount;
    const bonus = bingoCheck.isFourCorners ? CONFIG.FOUR_CORNERS_BONUS : 0;
    const totalPrize = basePrize + bonus;
    const houseEarnings = commissionPerPlayer * playersCount;
    ...
    // 4. Create transactions (parallel)
    const [winTransaction, houseTransaction] = await Promise.all([
      models.Transaction.create({
        type: bingoCheck.isFourCorners ? 'WIN_FOUR_CORNERS' : 'WIN',
        userId: userId,
        userName: userName,
        amount: totalPrize,
        room: room,
        description: `Bingo win${bingoCheck.isFourCorners ? ' (Four Corners Bonus)' : ''}`,
      ...
      models.Transaction.create({
        ...
        room: room,
        description: `Commission from ${playersCount} players`
      })
    ]);
    
    // 5. Store players list for notifications
    const playersInRoom = [...updatedRoom.players];
    
    // 6. Clear game timer
    cleanupRoomTimer(room);
    
    // 7. Reset room for next game
    updatedRoom.players = [];
    updatedRoom.takenBoxes = [];
    updatedRoom.status = 'waiting';
    updatedRoom.calledNumbers = [];
    updatedRoom.currentBall = null;
    updatedRoom.ballsDrawn = 0;
    updatedRoom.startTime = null;
    await updatedRoom.save();
    
    // 8. Prepare game over data WITH WINNING PATTERN
    const gameOverData = {
      room: room,
      winnerId: userId,
      winnerName: userName,
    ...
    console.log(`✅ Bingo processed: ${userName} won ${totalPrize} ETB`);
    
    // 9. BROADCAST WIN IMMEDIATELY
    io.emit('gameOver', gameOverData);
    
    // 10. Update balances for all players
    for (const playerId of playersInRoom) {
      if (playerId !== userId) {
        // Update other players (remove from room)
      ...
      for (const [socketId, uId] of socketToUser.entries()) {
        if (uId === playerId) {
          ...
          if (playerSocket && playerSocket.connected) {
            // Send balance update
            if (uId === userId) {
              playerSocket.emit('balanceUpdate', userUpdate.balance);
            }
          }
        }
      }
    ...
    // 11. Clear boxes for next game
    broadcastTakenBoxes(room, []);
    io.emit('boxesCleared', { room: room, reason: 'bingo_win' });
    
    // 12. Update displays
    broadcastRoomStatus();
    updateAdminPanel();
    
    logActivity('BINGO_WIN_FAST', { 
      userId, 
      userName, 
      room, 
      prize: totalPrize,
      players: playersCount,
      patternName: bingoCheck.patternName
    });
    
  } catch (error) {
    console.error('❌ Error processing bingo win:', error);
  }
}
      ...
      io.on('connection', (socket) => {
        ...
        socket.on('claimBingo', async (data, callback) => {
          try {
            ...
            // 1. IMMEDIATE RESPONSE - don't wait for processing
            if (callback) {
              callback({ 
                success: true, 
                message: 'BINGO claim received!',
                timestamp: Date.now()
              });
            }
            
            // 2. Use unique claim ID to prevent double processing
            const claimId = `${userId}_${room}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
            const claimKey = `claim_${claimId}`;
            
            // Check if same user already has a claim in progress for this room
            const existingUserClaim = Array.from(processingClaims.keys()).find(key => 
              key.includes(`${userId}_${room}`)
            );
            
            if (existingUserClaim && Date.now() - processingClaims.get(existingUserClaim) < 5000) {
              console.log(`⚠️ Duplicate claim prevented for user ${userId} in room ${room}`);
              return;
            }
            
            // 3. Set claim lock with short timeout
            processingClaims.set(claimKey, Date.now());
            
            // 4. Do minimum validation first
            const user = await models.User.findOne({ userId: userId });
            if (!user) {
            ...
            if (!roomData || !roomData.players.includes(userId)) {
              processingClaims.delete(claimKey);
              return;
            }
            
            // 5. Fast local bingo check
            ...
            // 6. QUICK WIN PROCESSING - Do heavy operations after sending response
            
            // Store win data for async processing
            const winData = {
              claimId,
              userId,
              userName: user.userName,
              ...
              markedNumbers,
              bingoCheck,
              roomDataId: roomData._id,
              playersCount: roomData.players.length,
              ballsDrawn: roomData.ballsDrawn,
              timestamp: Date.now()
            };
            
            // 7. IMMEDIATELY send "processing" state to prevent double clicks
            const winningPatternName = bingoCheck.patternName;
            const isFourCornersWin = bingoCheck.isFourCorners;
            
            // Send immediate visual feedback
            socket.emit('bingoProcessing', {
              claimId,
              patternName: winningPatternName,
              isFourCorners: isFourCornersWin
            });
            
            // 8. Process win in background WITHOUT blocking
            setTimeout(async () => {
              try {
                await processBingoWin(winData);
              } catch (error) {
                console.error('Error processing bingo win:', error);
              } finally {
                // Clean up claim lock
