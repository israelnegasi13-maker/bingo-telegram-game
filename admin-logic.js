...
function connectSocket() {
    ...
    try {
        ...
        if (state.connectionAttempts < state.maxConnectionAttempts) {
            setTimeout(() => {
                console.log(`🔄 Retrying connection (attempt ${state.connectionAttempts + 1}/${state.maxConnectionAttempts})...`);
                connectSocket();
            }, 2000);
        }
    }
...
function adminLogin() {
    ...
    if (!state.socket || !state.socket.connected) {
        ...
        showToast('Establishing connection to server...', 'info');
        ...
        setTimeout(() => {
            if (state.pendingPassword && (!state.socket || !state.socket.connected)) {
                console.log('⏰ Connection timeout - no response from server');
                showToast('Connection timeout. Please check if server is running.', 'error');
                state.pendingPassword = null;
...
window.manualLogin = function(password) {
    if (!password) {
